...
void ncp_usart_init()
{
  ...
  initData.txPort = BSP_UARTNCP_TX_PORT;
  initData.txPin = BSP_UARTNCP_TX_PIN;
  initData.rxPort = BSP_UARTNCP_RX_PORT;
  initData.rxPin = BSP_UARTNCP_RX_PIN;
  initData.uartNum = NCP_USART_USART;
  ...
  // This is workaround for xG21 current leaking issue from PA5
  GPIO_PinModeSet(gpioPortA, 5, gpioModePushPullAlternate, 1);
  GPIO->P[0].CTRL |= GPIO_P_CTRL_DINDISALT;
#endif
...
static void ncp_enable_deep_sleep()
{
  GPIO_PinModeSet(NCP_WAKEUP_PORT, NCP_WAKEUP_PIN, gpioModeInput, NCP_WAKEUP_POLARITY);
...
bool ncp_handle_event(struct gecko_cmd_packet *evt)
{
  ...
  switch (BGLIB_MSG_ID(evt->header)) {
    case gecko_evt_system_external_signal_id:
      ...
      if (evt->data.evt_system_external_signal.extsignals & NCP_USART_SECURITY_SIGNAL) {
        gecko_send_system_error(bg_err_application_encryption_decryption_error, 0, NULL);
        evt_handled = true;
      }
      break;
    default:
